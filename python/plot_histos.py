...
def plot_ptbin_stack(hists, category, year, outdir, save_individual, region):
    ...
    for i in range(len(pt_edges) - 1):
        ...
        fig.savefig(
            f"{outdir}/{year}_{category}_{region}_ptbin{pt_low}_{pt_high}.png",
            dpi=300,
            bbox_inches="tight",
        )
...
def main(args):
    ...
    for category in ["pass", "fail"]:
        print(f"Plotting histograms for category: {category}, year: {year}...")
        plot_ptbin_stack(
            histograms,
            category,
            year,
            ...
            region=args.region,  # CHANGED: Pass region
        )
